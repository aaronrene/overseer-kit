...
def _build_post_sync_entries(
    *,
    ...
    """Build full manifest entries after sync, retaining out-of-scope rows verbatim."""
    ...
    for item in sorted(rendered, key=lambda row: row.destination):
        ...
        if not in_scope and dest in prior:
            entries.append(prior[dest])
            continue
...
def run_sync(args: Namespace, ctx: CliContext) -> int:
    ...
    blocking_conflicts = [row for row in in_scope if row.is_conflict]
    ...
    if not writes_needed and lock.kit_version == kit_version():
