...
| **SD-21** | 2026-07-29 | **Finish-mode land hygiene after BV `pass` is Tier 1.** When build-verification verdict is `pass`, the feature branch is complete, and the diff does **not** flip live posture/env, add secrets, touch real money, or set Delegation write env: agents **just do** Muse merge/cherry-pick → `main` → `muse-bridge-deploy` → merge green `muse-mirror`→GitHub `main` (SD-14 path only). Still forbidden: `git push origin main`, feature→GitHub-`main`, staging push, blind `--auto` merge without green checks. | Removes false “ask for every land” friction that was blocking hygiene; keeps SD-14 mirror-only and all live/security gates Tier 3. | `policy/tiers.yaml`; muse+git-mirror consumers |
