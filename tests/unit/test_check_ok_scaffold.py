...
def test_render_includes_frozen_true_and_seven_tiers() -> None:
    ...
    assert "phase: check-ok-demo" in body
...
def test_footprint_includes_check_ok_skill_and_rule(git_only_config) -> None:
    dests = {f.destination for f in resolve_footprint(git_only_config)}
    assert ".cursor/skills/check-ok/SKILL.md" in dests
    assert ".cursor/skills/check-ok/SIDE-CHECK-TEMPLATE.md" in dests
    assert ".cursor/rules/check-ok-thinking.mdc" in dests
    # Claude Code twin (same source bytes)
    assert ".claude/skills/check-ok/SKILL.md" in dests
    assert ".claude/skills/freeze-review/SKILL.md" in dests
...
def test_skill_documents_same_engine_as_review(git_only_config) -> None:
    ...
    skill = files[".cursor/skills/check-ok/SKILL.md"]
    assert "ok review --freeze" in skill
    assert "ok check-ok" in skill
    assert "build-verification-review" in skill
    assert "docs.lanes" in skill
    assert files[".claude/skills/check-ok/SKILL.md"] == skill
