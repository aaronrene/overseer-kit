...
def test_e2e_status_exit_code_unchanged_by_advisory(tmp_path: Path, capsys) -> None:
    """§NXP.8 e2e: N4 advisory must not change --exit-code precedence."""
    write_config(tmp_path, "config-git-only.yaml")
    docs = tmp_path / "docs"
    docs.mkdir(parents=True, exist_ok=True)
    (docs / "OVERSEER-HANDOVER.md").write_text(
    ...
    # Bare names → advisory present, but exit follows pre-existing conditions only.
    code = run_cli(
        ["status", "--exit-code"],
        cwd=tmp_path,
    ...
    # Advisory may appear; exit is not raised solely by board naming.
    # git-only fixture with empty footprint digest may already exit 2/6 for other reasons —
    # assert naming itself is not the gate by checking advisory is warn text only.
    if "board naming:" in out:
        assert code in {0, 2, 3, 6, 35}  # any pre-existing; naming alone wouldn't invent a code
    # Stronger: same fixture without --exit-code is 0 for human path
    code0 = run_cli(
        ["status"],
        cwd=tmp_path,
        runner=git_status_runner(),
