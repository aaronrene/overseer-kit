...
from tools.check_if_ok.scaffold import render_side_check_markdown, scaffold_side_check
...
def test_cli_rejects_escape(tmp_path: Path) -> None:
    ...
    code = run_cli(
        ["check-if-ok", "--path", "../../etc/passwd", "--scaffold-only"],
        cwd=tmp_path,
