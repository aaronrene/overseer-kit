"""Active-slice Mode D independent second reviewer surface (§ISR.6)."""
...
REQUIRE_REMEDIATION = (
    "second chat: append independent_second_review then "
    "ok honesty-status --independent-second-review PHASE"
)


@dataclass(frozen=True)
class IndependentSecondReviewerGateReport:
    """Result of the active-slice independent second reviewer probe."""
...
def build_independent_second_reviewer_gate(
    ...
    *,
    ...
    """Run active-slice Mode D when honesty warn/require is enabled."""
    require = config.honesty.require_independent_second_reviewer
    if not config.honesty.enabled or require not in {"warn", "require"}:
        return IndependentSecondReviewerGateReport(skipped=True, ok=True)
    ...
    if not _should_run_isr_gate(phase_id, handover, roadmap):
        return IndependentSecondReviewerGateReport(skipped=True, ok=True)
    ...
    if matched:
        return IndependentSecondReviewerGateReport(
            skipped=False,
            ok=True,
            mode=require,
            matched=True,
    ...
    if require == "require":
        return IndependentSecondReviewerGateReport(
            ...
            matched=False,
            phase_id=phase_id,
            message="missing independent_second_review ledger entry for active Auto slice",
            token="missing_independent_second_review",
            remediation=REQUIRE_REMEDIATION,
        )
    ...
    return IndependentSecondReviewerGateReport(
        ...
        matched=False,
        phase_id=phase_id,
        message="warning: no matching independent_second_review entry for active Auto slice",
    )


def independent_second_reviewer_gate_payload(
    report: IndependentSecondReviewerGateReport,
) -> dict | None:
    if report.skipped:
        return None
    payload: dict = {
...
def format_independent_second_reviewer_gate_line(
    report: IndependentSecondReviewerGateReport,
) -> str | None:
    if report.skipped or report.message is None:
        return None
    return f"independent_second_reviewer_gate: {report.message}"
...
def _should_run_isr_gate(phase_id: str, handover: str | None, roadmap: str | None) -> bool:
    corpus = (handover or "") + "\n" + (roadmap or "")
