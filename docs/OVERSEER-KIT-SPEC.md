...
| `ok status` | Report: kit version, `version.lock` version, drift (behind/ahead), VCS regime, dirty tree, last governance-sync. Read-only. **PMHF additive:** JSON key `land_closeout` (`{state, ok, message, remediation, land_phase, freshness_ok, d1, optional_pr_merged}`); `--exit-code` folds `land_closeout.ok` into the existing `2` tier (no renumbering; `land_a_in_progress` stays ok so waiting for merge never false-fails). Default status path never invokes `gh` for land closeout (`probe_merged_pr=False`). **LT additive:** JSON keys `footprint_coverage`, `ide_workspace_hint`, `optional_feature_tips`, optional `verification_evidence_gate`; `--exit-code` folds `footprint_coverage.ok` and `verification_evidence_gate.ok` (require mode only) into the existing `2` tier. Frozen: `docs/archive/phases/PHASE-LT-LOOP-TIGHTENING.md`. **ISR additive:** optional JSON key `independent_second_reviewer_gate` (absent when the probe skips); `--exit-code` folds `independent_second_reviewer_gate.ok` (require mode only) into the existing `2` tier. Frozen: `docs/archive/phases/PHASE-ISR-INDEPENDENT-SECOND-REVIEWER.md` §ISR.6. | No | Yes |
...
| `ok next [--lane NAME]` | **Print NEXT** (ONS): read-only extract of the KH1 paste-ready fence from the config-driven handover; prints under `## CURRENT NEXT — paste this`. Synonym: `ok governance-sync --print-next` (short-circuits; no R1–R5 / patches / commits). Fail-closed exit `37` when handover/fence malformed. Not `ok workspace check-next`. Frozen: `docs/archive/phases/PHASE-ONS-OPERATOR-NEXT-SURFACING.md`. | No | Yes |
| `ok handover-compact [--dry-run\|--write] [--keep N] [--lane NAME]` | Archive old handover change-log bullets to `{docs}/archive/handover/CHANGE-LOG.md`. Default dry-run when neither flag given. `--keep` default 15 (minimum 5). Does not regenerate NEXT. Frozen: `docs/archive/phases/PHASE-LT-LOOP-TIGHTENING.md`. | Yes (with `--write` only) | Yes |
...
| `ok honesty-status --hook HOOK --artifact PATH [--producer-session ID] [--json]` | L2 co-requirement check (K10): require a passing independent `verdict` for artifact SHA before board/handoff/register hooks. Module gate: `honesty.enabled` must be true. Exit extensions: `20` missing verdict, `4` hook not enabled / module off. **ISR additive:** Mode D `--independent-second-review PHASE_ID` with optional `--producer-session` / `--frozen-spec`; exit `38` + `missing_independent_second_review` when `require_independent_second_reviewer: require` and no match. Frozen: `docs/archive/phases/PHASE-ISR-INDEPENDENT-SECOND-REVIEWER.md`. | No | Yes |
