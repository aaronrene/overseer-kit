...
Contract: `docs/archive/phases/PHASE-K8-MULTI-LANE-DOCS-CONTRACT.md`. Fixture shape:
...
| `post_land_sync.enabled` | Opt-in (default **off**): after a successful `ok pr-land` merge, ff-only sync the local main checkout from `vcs.git.remote` (kit `docs/archive/phases/PHASE-PLS-POST-LAND-MAIN-SYNC.md`) |
...
VideoFactory mirror: `jobs/pr_land_after_checks.py`. Spec: kit `docs/archive/phases/PHASE-PR-LAND-AFTER-CHECKS.md`.
