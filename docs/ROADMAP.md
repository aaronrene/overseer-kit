...
| **Track P / P1** | Auto | **TODO** | Build the frozen `provenance` envelope: extend `tools/honesty/{canonical,validate,ledger}.py`, add `require_agent_signature` config + exit codes `25`/`26`, Ed25519 verify path, seven-tier matrix per §P0.8. **Gated on `/build-verification-review` before DONE.** No social features. |
