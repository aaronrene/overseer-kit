...
| **Track P / P-cost build** | Auto | **TODO** | Build mechanically against frozen `docs/PHASE-TRACK-P-P-COST-AWARENESS.md`: optional `cost_class` key on `model_tiers` (closed vocabulary `free\|low\|moderate\|high`; recognized key, not rejected as unknown); `paid_step_before_spend` derivation; additive `cost_class` + `paid_step_before_spend` on `overseer route` output; optional `cost_awareness:` config (`enabled` default `false`, `surfaces` default `[status, governance-sync]`); active-slice spend-awareness surface on `overseer status` (+ `--json` key) and the `governance-sync` footer + a static reminder line in `templates/OVERSEER-HANDOVER.template.md`; exit `32` on `overseer route`; seven-tier §PC.9 tests. Kit = cost-awareness rule-holder, runtime = spender. **Gated on `/build-verification-review`.** |
