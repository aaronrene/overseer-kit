...
| **Track P / P-deploy deployment gate** | Thinking | **TODO** | Live-deploy sibling of build-verification: freeze kit-side record/gate for deploy/health claims before "shipped" → DONE. Kit never deploys. Promoted from exploration backlog after Track O close-out. |
...
| **P-deploy — deployment gate** | Promoted → Track P / P-deploy (Thinking TODO) after Track O close-out. | Kit records/gates the *claim*; it never performs the deploy. |
| **Hosted governance dashboard** | Read-only web view of a repo's governance state (roadmap/handover/gates) sourced from GitHub/MuseHub APIs — for "see my org's governance at a glance" without a local install. Distinct from Track Q: hosted/remote and read-only; cannot touch local repos, git, or muse. | Read-only; the authoritative workflow stays local (repo + git/muse). |
...
| **Q3-release — desktop installers** | CI publishes signed Tauri artifacts (`.dmg` / `.msi` / `.AppImage`) so Path C is not build-from-source only. | Packaging/distribution; no engine redesign. |
