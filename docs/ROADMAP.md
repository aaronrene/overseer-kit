...
| **Track O / O3 Stage 3 upgrade-regime build** | Auto | **TODO** | Build `ok upgrade-regime` + runbook + product-contract/harness retarget against frozen `docs/PHASE-TRACK-O-O2-STAGE3-UPGRADE-CEREMONY.md`. Seven tiers §O2.9; `/build-verification-review` before DONE. No signup UI; no live consumer init; no C8 merge. |
...
| --- | --- | --- |
| **Track O / O-onboard — Normie custody funnel** | Promoted → O0 (freeze DONE) + O1 (Auto product contracts DONE) + O2 (Stage 3 ceremony freeze DONE); O3 Auto `ok upgrade-regime` queued. | Kit owns regimes + CLI; Scooling/Knowtation own signup UX. No MuseHub-only baseline. |
...
- `docs/TRACK-O-NORMIE-CUSTODY-PRODUCT-CONTRACT.md` — Track O / O1 normative product contract (Stages 1–4 + boundary/rejection; Stage 3 one-click still blocked until O3 BV)
