...
| **K5b Freeze reviewer build** | Auto | **DONE** (on `main` via PR #5 — **not cleared**) | `overseer review --freeze` + nested `freeze_contract.reviewer` parse (legacy string normalization) + `reviewer_models` in `policy/model-labels.yaml` + `tools/freeze_reviewer/` engine (injectable local\|api; `fallback: human` fail-closed) + §K5.9 report + stamp write + Automation templates + seven-tier tests. Premature merge before K5b-r (F6); clearance requires K5b-r2 `pass` + `fix/k5b-r-findings`. |
| **K5b-r Merge gate review** | Thinking | **WIP** (round 1 **`blocked`** → round 2 pending) | Round 1: F1–F6 cited on PR #5. F1–F5 fixed on `fix/k5b-r-findings` (170 tests; dogfood pass). **K5b-r2** Thinking re-review required before clearance. |
| **9A-5 Governance Hygiene Agent** | Auto | **TODO** | `overseer governance-sync [--dry-run]` against kit VCS adapter — **blocked until K5b-r2 `pass` + fix merge** |
