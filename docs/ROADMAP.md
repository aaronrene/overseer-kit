...
| **Track P / P0** | Thinking | **WIP** | **Agent identity & signed provenance** freeze — contract drafted in `docs/PHASE-TRACK-P-P0-AGENT-PROVENANCE.md`. Adds optional `provenance` envelope (`agent_id`/`model_id`/Ed25519 `sig`) to the honesty ledger; shared primitive with the Muse social domain; soft under git-only, hard under Muse. **Pending freeze-review `pass`** before P1 Auto build. Social stays consumer-only (Muse protocol + Schooling UI). |
