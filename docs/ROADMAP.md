...
| **Track O / O0 Normie custody funnel** | Thinking | **TODO** | Freeze muse-first / optional GitHub backup / Knowtation bind path for non-devs. Draft seed: `docs/PHASE-TRACK-O-O0-NORMIE-CUSTODY-FUNNEL.md` (**not frozen** until Thinking review `pass`). Kit = regimes+CLI; products own wizards. |
...
| --- | --- | --- |
| **Track O / O-onboard — Normie custody funnel** | Muse-first personal space (no Git required to start); optional one-click GitHub backup (`muse+git-mirror`); optional Knowtation vault bind; Scooling as optional entry product, not kit dependency. Draft seed queued as **Track O / O0** Thinking. | Kit owns regimes + CLI; Scooling/Knowtation own signup UX. No MuseHub-only baseline. |
...
- `docs/PHASE-TRACK-O-O0-NORMIE-CUSTODY-FUNNEL.md` — Track O / O0 Normie custody funnel (**draft seed**; Thinking freeze required before `frozen: true`)
