...
| **Track P / P-route build** | Auto | **TODO** | Build mechanically against the frozen `docs/PHASE-TRACK-P-P-ROUTE-MODEL-ROUTING.md`: vendor `policy/model-routing.yaml`; extend (do not fork) `policy/model-labels.yaml` with `model_tiers`; optional `model_routing:` config parse; read-only `overseer route` (resolve / `--validate` / explain — no dispatch, no network, no key); exit codes `30`/`31`; seven-tier tests green. **Gated on `/build-verification-review`.** |
